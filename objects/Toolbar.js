class Toolbar extends Phaser.GameObjects.Container {
    ...
    display() {
        ...
        var coinIcon = this.currentScene.add.sprite(150, 30, "coins").setOrigin(0.5, 0.37);
        coinIcon.scaleX = .085;
        coinIcon.scaleY = .085;
        this.aGrid.placeAtIndex(2, coinIcon);

        this.coinText = this.currentScene.add.text(165, 30, playerStats.coins).setOrigin(0.2, 0.1);
        this.aGrid.placeAtIndex(2.5, this.coinText);

        var pickaxeIcon = this.currentScene.add.sprite(690, 30, "pickaxe").setOrigin(0.8, 0.4);
        pickaxeIcon.scaleX = .075;
        pickaxeIcon.scaleY = .075;
        this.aGrid.placeAtIndex(9, pickaxeIcon)

        this.pickaxeText = this.currentScene.add.text(0,0, playerStats.pickAxePower).setOrigin(0, 0.1);
        ...
        this.backpackText = this.currentScene.add.text(0, 0, `${playerStats.currentBackpackItems.length}/${playerStats.backPackCapacity}`).setOrigin(0.2, 0.1);
