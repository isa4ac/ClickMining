class Shop extends Phaser.Scene {
  ...
  getSublabel(key) {
    if (key === "pickaxeUpgrade") {
      ...
      if (this.playerStats.autoMinerSpeed > 0) {
        return `Autominer Will Strike Every ${(this.playerStats.autoMinerSpeed - 500) / 1000} Seconds`
      } else {
        return 'Max Autominer Speed Reached!'
      }
    }
  }
    ...
    let sellRowBackground = this.add.rectangle(0, 0, 500, rowHeight, 0x7ae08e);
    sellRowBackground.setOrigin(0);
    sellRowBackground.setInteractive();
    sellRowBackground.on('pointerdown', () => {
      // sell logic ran here
      this.sellItems();
    });
